...
moje_kuchyne = {
  "Pánvička": {
    ...
    "Použité": True,
    "Značka": "Teleshop",
    "Součásti": {"hlavní část", "držadlo", "díra v držadle"},
    "Barvy": ("Černá", "Stříbrná", "Červená"),
    "Datum upustění na zem": ["1.1.1", "12.6.2005", "28.10.6969", "3.8.9999"]
  },
  "Hrnec": {
    "Stáří":  41896,
    "Použité": False,
    "Značka": "Lidl",
    "Součásti": {"ucho 1", "ucho 2", "poklice", "hlavní část"},
    "Barvy": ("Stříbrná", "Modrá"),
    "Datum upustění na zem": []
  },
  "Talíř": {
    "Stáří":  12894189,
    "Použité": True,
    "Značka": "Ikea",
    "Součásti": {"Kulatý základ", "Kulaté kraje"},
    "Barvy": ("Tmavě modrá", "Bílá"),
    "Datum upustění na zem": ["23.13.1234", "30.2.4321"]
  }
}
moje_kuchyne["Mrazák"] = {
    "Stáří":  45615615,
    "Použité": True,
    "Značka": "Datart",
    "Součásti": {"Mrazící box", "Šuplík 1", "Šuplík 2", "Dveře"},
    "Barvy": ("Hnědá", "Bílá"),
    "Datum upustění na zem": ["23.13.1234", "30.2.4321"]
}
moje_kuchyne.pop("Talíř")
print(moje_kuchyne)
print("------------------------------------------------------------------------------------------------------------------------------------------------------------------")
print("Náčiní        Stáří          Použité       Značka        Součásti                                Barvy                      Datum upustění na zem")
print("------------------------------------------------------------------------------------------------------------------------------------------------------------------")
[print(f"{n.ljust(14)}"
       f"{(str(moje_kuchyne[n]['Stáří'])+' let').ljust(15)}"
       f"{'Ano'.ljust(14) if moje_kuchyne[n]['Použité'] else 'Ne'.ljust(14)}"
       f"{str(moje_kuchyne[n]['Značka']).ljust(14)}"
       f"{''.join([s+', ' for s in moje_kuchyne[n]['Součásti']])[:-2].ljust(40)}"
       f"{''.join([s+', ' for s in moje_kuchyne[n]['Barvy']])[:-2].ljust(27)}"
       f"{''.join([s+', ' for s in moje_kuchyne[n]['Datum upustění na zem']])[:-2]}"
       ) for n in moje_kuchyne]
print("-----------------------------------------------------------------------------------------------------------------------------------------------------------------")
'''
Slovník myfamily
---------------------------------------------
