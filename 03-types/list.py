...
print(hundreds)
ascii = [r.choice(az) for x in range(50)]
print(ascii)
...
print(f'\n*************************************\nCvičení 3\n*************************************')
persons += [("Petr", 45, "muž"), ("James", 32, "muž"), ("Anna", 67, "žena"), ("Kateřina", 23, "žena")]
print(persons)
women = list(filter(lambda p: p[2] == "žena", persons)) or [p for p in persons if p[2] == "žena"]
print(women)
for woman in women: print(f"{woman[0]}\n{'-' * len(woman[0])}")
ipeople = list(filter(lambda p: "i" in p[0].lower(), persons)) or [p for p in persons if "i" in p[0].lower()]
print(ipeople)
string = "".join([f"{i};{p[0]};{p[1]};{p[2]}\n" for i, p in enumerate(ipeople)])
print(string)
