...
# first_name[0] = 'R'
'''
Proto jedinou cestou, jak vytvářet nové řetězce, je jejich kom
# print('R' + first_name[1:])binování, které je velice jednoduché a přitom efektivní:
# '''
...
def cz_to_data(d):
    return f"{d.split('.')[2].strip()}-{d.split('.')[1].strip()}-{d.split('.')[0].strip()}"


def str_to_ident(string, options="python"):
    return "".join([x.lower()+"_" for x in string.split()])[:-1] if options.lower() == "python" else "".join([x.capitalize() if i > 0 else x.lower() for i, x in enumerate(string.split())]) if options.lower() == "javascript" else None
...
def passwords(count):
    return ["".join([r.choice(az[int(len(az)/2):]) for x in range(3)])+"".join([r.choice(az[:int(len(az)/2)]) for x in range(3)])+f"{r.choice(p)}"+"".join([r.choice(d) for x in range(3)]) for pswd in range(count)]



print(cz_to_data("12. 10. 2020"))
print(str_to_ident("To je proměnná v Pythonu"))
print(str_to_ident("To je proměnná v JavaScriptu", options="javascript"))
print(passwords(5))