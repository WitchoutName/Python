...
# & | ^ ~ << >>
name = "Tomáš Foltyn"
myself_binary = int("".join(["0" if x in "aáeéěiíyýoóuúů" else "1" for x in "".join(name.split()).lower()[:8]]), 2)

print(myself_binary)
myself_binary >>= 2
print(myself_binary)
print(f"Binární součin čísla {bin(myself_binary)} a {bin(int('1A', 16))} je {bin(int('1A', 16) * myself_binary)}")
